    ...
    >
      <div className="space-y-5 flex-1">
        ...
        <div className="flex justify-between items-center border-b border-gray-200 pb-2 text-[9px] text-gray-400 uppercase font-semibold">
          <span>Formato A4 • Documento Oficial de Cobrança</span>
        ...
        {/* INSTRUÇÕES PIX, RECEBEDOR E BANCO */}
      ...
      <div className="border-t border-slate-200 pt-3 text-center space-y-1 text-[10px] text-slate-500 mt-auto">
        <p className="font-semibold text-slate-700">
          Documento Oficial gerado eletronicamente por {entityConfig?.name || 'UniOn Sistema de Gestão'}.
        </p>
        <p>
...
export default function BillingChargesTab({
  ...
  const handleOpenSendModal = (charge: Charge) => {
    ...
    const pixValue = charge.pixKey || entityConfig?.cnpj || '00.000.000/0001-00';
...
Consta em nosso sistema uma cobrança emitida por ${receiverText}:

📌 Cobrança: ${charge.title}
🔢 Código: ${charge.codeNumber}
💰 Valor: R$ ${charge.amount.toFixed(2)}
📅 Vencimento: ${dateToBRL(charge.dueDate)}
                  ...
                  return (
                    <tr key={charge.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="py-3.5 px-4 font-medium">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-[10px] text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md shrink-0">
                            {charge.codeNumber}
                          </span>
                          <div>
                            <span className="text-white font-bold block">{charge.title}</span>
                            {charge.paymentMethod && (
                              <span className="text-[10px] text-gray-500">Via {charge.paymentMethod}</span>
                            )}
                          </div>
                        </div>
                      </td>
