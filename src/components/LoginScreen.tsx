...
import { Lock, Mail, Users, Building, AlertCircle, Sparkles, Clock, Eye, EyeOff } from 'lucide-react';
import { Associate, EntityConfig, AdminConfig } from '../types';
...
export default function LoginScreen({ 
  ...
  useEffect(() => {
    const key = activeTab === 'admin' ? 'union_remember_admin' : 'union_remember_associate';
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    const storageKey = activeTab === 'admin' ? 'union_remember_admin' : 'union_remember_associate';
    if (rememberMe) {
      ...
      >
        ...
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-white font-sans">
            {entityConfig?.name || 'UniOn'}
          </h2>
          <p className="mt-2 text-xs text-gray-400">
            {entityConfig?.acronym ? `Portal de Gestão - ${entityConfig.acronym}` : 'Sistema de Gestão de Associações'}
          </p>
        </div>
        ...
        <div className="flex p-1 bg-[#1a1a1a] rounded-xl border border-white/5 gap-1">
          <button
            ...
            className={`flex-1 py-2 text-[11px] font-bold rounded-lg transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer ${
              activeTab === 'admin'
